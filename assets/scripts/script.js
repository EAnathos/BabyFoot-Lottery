...
const probabilitiesModal = document.getElementById("probabilitiesModal");
const probabilitiesList = document.getElementById("probabilitiesList");
const probabilitiesFilter = document.getElementById("probabilitiesFilter");
const probabilitiesCount = document.getElementById("probabilitiesCount");
...
function showProbabilitiesLoadError() {
...
function getRarityLabel(effect) {
  if (effect.isNoWayy) {
    return "no-wayy";
  }
...
function getRarityDisplay(effect) {
  if (effect.isNoWayy) {
    return "No Wayyy";
  }
  if (effect.isLegendary) {
...
function renderProbabilities() {
  ...
  const activeFilter = probabilitiesFilter?.value || "all";
  ...
  probabilitiesList.classList.add("probabilities-list");
  probabilitiesList.innerHTML = "";

  if (probabilitiesCount) {
    const countLabel = sortedEffects.length === 1 ? "effet" : "effets";
    probabilitiesCount.textContent = `${sortedEffects.length} ${countLabel}`;
  }
...
if (probabilitiesToggle && probabilitiesModal) {
  const closeProbabilities = () => {
    probabilitiesModal.classList.add("is-hidden");
    probabilitiesToggle.setAttribute("aria-expanded", "false");
  };

  const openProbabilities = () => {
    probabilitiesModal.classList.remove("is-hidden");
    probabilitiesToggle.setAttribute("aria-expanded", "true");
  };

  probabilitiesToggle.addEventListener("click", () => {
    if (probabilitiesModal.classList.contains("is-hidden")) {
      renderProbabilities();
      openProbabilities();
    } else {
      closeProbabilities();
  ...
  if (probabilitiesFilter) {
    probabilitiesFilter.addEventListener("change", () => {
      renderProbabilities();
    });
  }

  probabilitiesModal.addEventListener("click", (event) => {
    const target = event.target;
    if (
  ...
  document.addEventListener("keydown", (event) => {
    if (
      event.key === "Escape" &&
      !probabilitiesModal.classList.contains("is-hidden")
    ) {
      closeProbabilities();
    }
  });
